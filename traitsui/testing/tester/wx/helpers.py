...
def get_wobject_registry():
    ...
    handlers = [
        (command.KeyClick, (lambda wrapper, interaction: key_click_text_ctrl(
             wrapper.target, interaction.key, wrapper.delay))),
        (command.KeySequence, (lambda wrapper, interaction: key_sequence_text_ctrl(
             wrapper.target, interaction.sequence, wrapper.delay))),
        (query.DisplayedText, lambda wrapper, _: wrapper.target.GetValue()),
    ]
    ...
    registry.register_handler(
        ...
        handler=lambda wrapper, _: mouse_click_button(wrapper.target, wrapper.delay)
    )
