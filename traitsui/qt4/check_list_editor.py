...
class SimpleEditor(EditorWithList):
    ...
    def create_control(self, parent):
        ...
        self.control.activated.connect(self.update_object) # TODO: check
    ...
    def update_object(self, text):
        """ Handles the user selecting a new value from the combo box.
        """
        value = self.values[self.names.index(text_type(text))]
        if not isinstance(self.value, str_types):
            value = [value]
        self.value = value
...
class TextEditor(BaseTextEditor):
    ...
    def update_object(self, event=None):
        ...
        """
        try:
            value = text_type(self.control.text())
            value = eval(value)
        except:
            pass
        try:
