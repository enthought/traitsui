...
def _validate_range_value(range_object, object, name, value):
    ...
    """
    # This function implements the same validation logic as in the method
...
class CSVListEditor(EditorFactory):
    ...
    def _make_text_editor(self, ui, object, name, description, parent,
        ...
        # Create an editor with the appropriate evaluation and formatting
        # functions.
        ed = TextEditor(evaluate=evaluate, format_func=fmt_func,
                        auto_set=self.auto_set, enter_set=self.enter_set)

        # Call the factory function to create the actual editor.
        if readonly:
            se = ed.readonly_editor(ui, object, name, description, parent)
        else:
            se = ed.simple_editor(ui, object, name, description, parent)

        # Hook up a listener on `object` so that the display is updated if
        # the list changes external to the editor.
        object.on_trait_change(se.update_editor, name + '[]')
